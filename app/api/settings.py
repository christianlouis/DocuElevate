...
def _safe_setting_value(key: str, value: Any, metadata: Dict[str, Any] | None = None) -> Any:
    """Return a value that is safe to expose through an admin response.

    Admin authorization is not permission to recover application secrets.  In
    particular, a database URL remains useful for diagnostics when its driver,
    host, port and database are visible, but its embedded password must never be
    returned to the browser or a configuration export.
    """
    if value is None:
        return None
        ...
        for key in SETTING_METADATA.keys():
            if hasattr(settings, key):
                ...
                current_settings[key] = {
                    "value": _safe_setting_value(key, value, metadata),
                    "metadata": metadata,
                }

        # Get settings stored in database
        raw_db_settings = get_all_settings_from_db(db)
        db_settings = {
            key: _safe_setting_value(key, value, get_setting_metadata(key)) for key, value in raw_db_settings.items()
        }
...
async def export_env_settings(
    ...
    Returns a downloadable plain-text diagnostic export. Sensitive values are
    always masked, including credentials embedded in database URLs. The export
    can be used as a configuration template, but masked values must be replaced
    through the normal secret/bootstrap flow before reuse.
    """
    ...
    try:
        ...
        lines = [
            ...
            "# Sensitive values are masked and must be replaced before reuse.",
            "",
        ]
        for env_key, value in export_data.items():
            safe_value = _safe_setting_value(env_key.lower(), value)
...
async def get_setting(key: str, request: Request, db: DbSession, admin: AdminUser):
    ...
    try:
        ...
        safe_value = _safe_setting_value(key, value, metadata)
        return SettingResponse(key=key, value=str(safe_value) if safe_value is not None else None, metadata=metadata)
    except Exception as e:
...
async def update_setting(
    ...
    try:
        ...
        return {
            ...
            "key": key,
            "value": _safe_setting_value(key, setting.value, metadata),
        }
...
async def put_setting(
    ...
    try:
        ...
        return {
            ...
            "restart_required": restart_required,
            "key": key,
            "value": _safe_setting_value(key, body.value, metadata),
        }
    except HTTPException:
        raise
    except Exception as e:
...
async def bulk_update_settings(updates: list[SettingUpdate], request: Request, db: DbSession, admin: AdminUser):
    ...
    for update in updates:
        try:
            ...
            if success:
                metadata = get_setting_metadata(update.key)
                results.append(
                    {
                        "key": update.key,
                        "value": _safe_setting_value(update.key, update.value, metadata),
                        "status": "success",
                    }
                )
            else:
