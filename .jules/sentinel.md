...
## 2026-08-10 - [Fix DOM-based XSS in file_detail.html]
**Vulnerability:** A DOM-based Cross-Site Scripting (XSS) vulnerability existed in `frontend/templates/file_detail.html` where untrusted file metadata (like `document_title` and `original_filename`) from the similar documents API was directly concatenated into an HTML string and injected into the DOM via `.innerHTML`.
**Learning:** Even when building complex UI components dynamically using template literals inside JavaScript (such as rendering lists of related items), all interpolated variables derived from external sources must be sanitized. String concatenation for HTML generation is highly susceptible to DOM-based XSS if any variable is overlooked.
**Prevention:** When constructing HTML strings dynamically inside loops, ensure that all interpolated variables derived from API data are individually wrapped in `escapeHtml()` before concatenation.
