...
## 2026-08-04 - [Fix DOM-based XSS in file_detail.html loadSimilarDocuments]
**Vulnerability:** A DOM-based Cross-Site Scripting (XSS) vulnerability existed in `frontend/templates/file_detail.html` where `title` and `filename` from the `/api/files/{file_id}/similar` endpoint were inserted directly into an HTML string via template literals and assigned to `contentDiv.innerHTML`. An attacker who could influence document titles or filenames could execute arbitrary JavaScript.
**Learning:** When constructing HTML strings dynamically using template literals inside loops (e.g., mapping over API array responses to build lists of UI components), all interpolated variables derived from untrusted API data must be individually wrapped in `escapeHtml()` prior to concatenation and assignment to `.innerHTML`.
**Prevention:** Always use the centralized `window.escapeHtml()` utility to sanitize user-controlled variables (like titles, filenames, and dates) before they are injected into an HTML string that will be rendered via `.innerHTML`.
