...
## 2026-07-14 - [Fix DOM-based XSS in comments.js by unifying escapeHtml]
**Vulnerability:** A DOM-based Cross-Site Scripting (XSS) vulnerability existed in `frontend/static/js/comments.js` due to a custom `_renderMentions` function that redefined HTML escaping using a series of `.replace()` calls for `&`, `<`, and `>` only. It failed to escape single and double quotes, and was inconsistent with the centralized sanitization implementation.
**Learning:** Re-implementing security-critical sanitization functions locally is a consistent source of bugs and missed edge cases. All dynamic outputs intended for DOM insertion should rely on a centralized, thoroughly tested sanitization utility to ensure all dangerous characters (`<`, `>`, `&`, `"`, `'`) are escaped.
**Prevention:** Always use the globally provided sanitization function (e.g., `window.escapeHtml`) rather than writing custom regex-based sanitization loops on a per-module basis.
