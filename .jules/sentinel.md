...
## 2026-04-02 - SSRF Bypass via HTTP Redirects
**Vulnerability:** The `process_url` endpoint in `app/api/url_upload.py` performed SSRF validation on the initial URL but used `httpx.AsyncClient(follow_redirects=True)` without validating intermediate redirect URLs. This allowed an attacker to bypass SSRF protections by providing an external URL that redirects to a private IP or cloud metadata endpoint (e.g., `http://169.254.169.254`).
**Learning:** Checking the initial URL is insufficient if the HTTP client automatically follows redirects. The target of every redirect must be subjected to the same security checks as the initial URL.
**Prevention:** When using `httpx.AsyncClient(follow_redirects=True)`, always implement an event hook (e.g., `event_hooks={"response": [verify_redirect]}`) to intercept responses and validate the `Location` header URL before the redirect is followed.
