...
## 2026-07-28 - [Fix Zip Slip in Bulk Download]
**Vulnerability:** A Zip Slip (path traversal) vulnerability existed in `app/api/files.py` (bulk download endpoint) where an untrusted user-controlled `original_filename` was used as the `archive_name` and written directly to a zip archive using `zf.write(file_path, archive_name)`. If a malicious filename contained relative paths like `../../`, a naive extraction could write files outside the intended extraction directory.
**Learning:** When creating zip archives, any user-controlled input (like uploaded filenames) used for the `arcname` (archive name) parameter is a potential vector for Zip Slip vulnerabilities. Relying on `os.path.basename` when extracting files is common, but it is equally important to sanitize names when *creating* archives so they are safe to extract anywhere.
**Prevention:** Always sanitize filenames before writing them into zip files (e.g., using `sanitize_filename` from `app.utils.filename_utils`) to ensure no path traversal sequences (like `..`) can be embedded within the zip entry.
