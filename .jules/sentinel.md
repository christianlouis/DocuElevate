...
## 2026-07-28 - [Fix Path Traversal in Automation Action Upload]
**Vulnerability:** The endpoint `/api/automation/actions/upload` was incorrectly calling `os.path.basename` on the user input filename, which does not properly sanitize paths across all environments (e.g. Windows paths executed on Linux), leading to potential path traversal vulnerabilities.
**Learning:** `os.path.basename` should not be used as a security mechanism for file name sanitization. It can easily be bypassed by combining it with custom sanitization functions depending on the OS context.
**Prevention:** Use a dedicated and secure path sanitization function like `sanitize_filename` on the full original path directly, rather than relying on standard library functions like `os.path.basename` to extract safe components.
