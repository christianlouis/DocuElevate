...
Webhook URLs, secrets, and subscribed events are configured per-webhook via the `/api/webhooks/` endpoints (admin access required). Each delivery includes a versioned payload envelope, event/version headers, an optional HMAC-SHA256 signature for verification, and is retried with exponential backoff on failure.
